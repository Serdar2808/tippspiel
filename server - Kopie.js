...
app.get('/api/wrapped/:userId', (req, res) => {
    const { userId } = req.params;
    const { token } = req.query;

    // Einfache Auth: Gültiges Token muss vorhanden sein.
    const requestingUser = db.prepare('SELECT id FROM users WHERE token = ?').get(token);
    if (!requestingUser) return res.status(403).json({ error: "Kein gültiges Token" });
    ...
    try {
        // Bester Treffer: Höchste Punktzahl. Bei Gleichstand, frühestes Spiel.
        const bestTip = db.prepare(`
            SELECT m.id as matchId, m.teamA, m.teamB, m.resultA, m.resultB, t.tipA, t.tipB, t.points
            FROM tips t
            JOIN matches m ON t.matchId = m.id
            WHERE t.userId = ? AND m.finished = 1 AND t.points IS NOT NULL AND t.tipA IS NOT NULL AND t.tipB IS NOT NULL AND m.resultA IS NOT NULL
            ORDER BY t.points DESC, (t.tipA + t.tipB) DESC, m.kickoff ASC
            LIMIT 1
        `).get(userId);

        // Schlimmster Fehltipp: 0 Punkte bei abgegebenem Tipp. Bei mehreren, das früheste Spiel.
        const worstTip = db.prepare(`
            SELECT m.id as matchId, m.teamA, m.teamB, m.resultA, m.resultB, t.tipA, t.tipB, t.points, (ABS(t.tipA - m.resultA) + ABS(t.tipB - m.resultB)) as score_diff
            FROM tips t
            JOIN matches m ON t.matchId = m.id
            WHERE t.userId = ? AND m.finished = 1 AND t.points = 0 AND t.tipA IS NOT NULL
            ORDER BY score_diff DESC, m.kickoff ASC
            LIMIT 1
        `).get(userId);

        // Lieblings-Ergebnis: Am häufigsten getipptes Ergebnis.
        const favoriteScore = db.prepare(`
            SELECT tipA, tipB, COUNT(*) as count
            FROM tips
            WHERE userId = ? AND tipA IS NOT NULL AND tipB IS NOT NULL
            GROUP BY tipA, tipB
            ORDER BY count DESC, tipA DESC, tipB DESC
            LIMIT 1
        `).get(userId);

        // Längste Serie mit korrekter Tendenz (>= 1 Punkt)
        const allTips = db.prepare(`
            SELECT t.points
            FROM tips t
            JOIN matches m ON t.matchId = m.id
            WHERE t.userId = ? AND m.finished = 1 AND t.tipA IS NOT NULL
        ...
        res.json({
            ...
            bestTip,
            worstTip,
            favoriteScore: favoriteScore ? { score: `${favoriteScore.tipA}:${favoriteScore.tipB}`, count: favoriteScore.count } : null,
            streak: longestStreak,
        });
    } catch (e) {
        console.error(`[Wrapped API] Fehler für User ${userId}:`, e);
        res.status(500).json({ error: "Interner Serverfehler beim Erstellen des Rückblicks." });
    }
});

// ── Head-to-Head Chart-Daten ──────────────────────────────────────────────────
app.get('/api/h2h', (req, res) => {
    ...
    try {
        // Holt die kumulierten Punkte pro Spieltag (gruppiert nach Kalendertag) für alle User.
        const rows = db.prepare(`
            WITH PointsPerDay AS (
                SELECT t.userId, DATE(m.kickoff / 1000, 'unixepoch') as match_day, SUM(t.points) as daily_points
                FROM tips t JOIN matches m ON t.matchId = m.id
                WHERE m.finished = 1 AND t.points IS NOT NULL
                GROUP BY t.userId, match_day
            )
            SELECT ppd.userId, u.name as userName, ppd.match_day,
                   SUM(ppd.daily_points) OVER (PARTITION BY ppd.userId ORDER BY ppd.match_day) as cumulative_points
            FROM PointsPerDay ppd JOIN users u ON ppd.userId = u.id
            ORDER BY ppd.userId, ppd.match_day
        `).all();

        const dataByUser = {};
        const allDays = [...new Set(rows.map(r => r.match_day))].sort();

        rows.forEach(row => {
            if (!dataByUser[row.userId]) {
                dataByUser[row.userId] = { label: row.userName, data: Array(allDays.length).fill(null), userId: row.userId };
            }
            const dayIndex = allDays.indexOf(row.match_day);
            if (dayIndex > -1) dataByUser[row.userId].data[dayIndex] = row.cumulative_points;
        });

        Object.values(dataByUser).forEach(userDataset => {
            let lastVal = 0;
            for (let i = 0; i < userDataset.data.length; i++) {
                userDataset.data[i] = userDataset.data[i] === null ? lastVal : (lastVal = userDataset.data[i]);
            }
        });

        res.json({ labels: allDays, datasets: Object.values(dataByUser) });
    } catch (e) {
